...
RSpec.describe 'Road Trip Request' do
  ...
  it 'can make call and return road trip data' do
    user = User.create(
                  {
  ...
  it 'can make call and return road trip data when the trip is longer than 24 hours' do
    user = User.create(
                  {
                    email: 'happypandas@gmail.com',
                    password: 'password',
  ...
  it 'can return specific data if the route is impossible' do
    user = User.create(
                  {
                    email: 'happypandas@gmail.com',
                    password: 'password',
    ...
    expect(roadtrip_info[:data][:type]).to be_a(String)
    expect(roadtrip_info[:data]).to have_key(:attributes)
    roadtrip_attrs = roadtrip_info[:data][:attributes]
    expect(roadtrip_attrs).to be_a(Hash)
    expect(roadtrip_attrs).to have_key(:start_city)
    expect(roadtrip_attrs[:start_city]).to be_a(String)
    expect(roadtrip_attrs).to have_key(:end_city)
    expect(roadtrip_attrs[:end_city]).to be_a(String)
    expect(roadtrip_attrs).to have_key(:travel_time)
    expect(roadtrip_attrs[:travel_time]).to be_a(String)
    expect(roadtrip_attrs).to have_key(:weather_at_eta)
    expect(roadtrip_attrs[:weather_at_eta]).to be_a(String)
  end

  it 'can return error message when no api_key is passed' do
    headers = {
              'Content-Type': 'application/json',
              'Accept': 'application/json'
              }
    ...
    roadtrip_info = JSON.parse(response.body, symbolize_names: true)
    expect(roadtrip_info).to be_a(Hash)
    expect(roadtrip_info).to have_key(:error)
    expect(roadtrip_info[:error]).to eq("Api Key is absent or invalid")
  end
end
